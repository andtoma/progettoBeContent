...
Class panel extends taglibrary {
...
///			$content .= "<td style=\"text-align:center; background-color: #FFDDBB; padding: 3px; width: 80px; font-size: 11px; height: 50px;\" colspan=\"{$span}\">";
	...
	function getSingleGrid($name,$data,$pars) {
		
		$content = "<table>\n";
		foreach($data as $key => $value) {
			
			$content .= "<tr>\n";
			$content .= "<td><input type=\"checkbox\" name=\"{$name}_{$value['value']}\" value=\"{$value['value']}\"></td><td>{$value['text']}</td>\n";
			$content .= "</tr>\n";
		}
		$content .= "</table>\n";
	...
	function getGrid($name,$data,$pars) {
		...
		switch ($pars['mode']) {
			...
			$n_gruppi = $pars['n_groups']; // $entityMain
			$n_servizi = $pars['n_services']; // $entitySecondary
