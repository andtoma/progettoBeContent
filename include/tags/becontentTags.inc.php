...
Class becontentTags extends taglibrary {
	...
	function getcomments($name, $data, $pars) {
		...
		$entity = $GLOBALS['database']->getEntityByName($pars['entity']);
		$comments = $GLOBALS['becontent']->comments[$entity->name];
	...
	function formatdate($name, $data, $pars) {
		...
		return aux::formatDate($data, $pars['mode']);
	...
	function seo_url($name, $data, $pars) {
		return aux::seo_url($data);
	}
	
	function email($name, $data, $pars) {
		return aux::email($data, $pars);
	}
	...
	function show($name, $data, $pars) {
		...
		*/
		...
		$filename = "{$GLOBALS['config']['cache_folder']}/".md5($query_string).".jpg";
		
		#echo filemtime($filename) + $GLOBALS['config']['cache_timeout'], "<br>", time();exit;
		
		if (!(file_exists($filename)) or (filemtime($filename) + $GLOBALS['config']['cache_timeout'] < time())) {
		#if (!(file_exists($filename))) {
			
			return "<img src=\"show.php?{$query_string}\" alt=\"\"><!-- normal img ".md5($query_string)."-->";
			#return "<img src=\"{$GLOBALS['config']['cache_folder']}/".md5($query_string).".jpg\" alt=\"\" /><!-- cached img -->";
		} else {
			return "<img src=\"{$GLOBALS['config']['cache_folder']}/".md5($query_string).".jpg\" alt=\"\" /><!-- cached img -->";
		}
