...
var pandora = {
	...
	adjustFooterPadding : function() {
		...
		$('.main-content-wrapper').css({
			paddingBottom : mainfooterwrapper.height()
		});
	},
	initUserAuthMenu : function() {
		$('.main-header .menu .account').not('._logged-in').click(function() {
			if ($('._user_auth').is(':visible')) {
				$('._user_auth .password').css('visibility', 'hidden');
				//hack to prevent "forget" from showing while runnin slideup animation
	...
	initHomepageSlider : function() {
		...
		$('#hompage-slider_content').touchwipe({
			wipeLeft : function() {
				$('#hompage-slider_content').cycle('next');
			},
			wipeRight : function() {
				$('#hompage-slider_content').cycle('prev');
			},
			wipeUp : function() {
				return false;
			},
